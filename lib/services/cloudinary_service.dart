...
class CloudinaryService {
  static const String cloudName = "dtzjhtowt"; // Cloud Name của bạn
  static const String uploadPreset = "global-chat"; // Upload preset của bạn

  static Future<String?> uploadFile(File file,
      {bool isVideo = false, bool isAudio = false}) async {
    String resourceType = isVideo ? 'video' : (isAudio ? 'raw' : 'image');
    final String apiUrl =
    ...
    if (response.statusCode == 200) {
      return jsonResponse["secure_url"]; // Trả về URL của file trên Cloudinary
    } else {
      print("Upload lỗi: ${jsonResponse['error']['message']}");
      return null;
    }
